...
public class Main {
    public static void main(String[] args) {
        ...
        Optional<Employee> opEmp = employeeDAO.findEmpByFName("Karan");
        Employee employee1 = opEmp.orElseGet(()-> Employee.builder().build());
        Employee employee2 = opEmp.orElseThrow(() -> new RuntimeException("Emp with name xyz doesn't exist" ));
        opEmp.ifPresent(e -> System.out.println(e.getSalary()));
        if (opEmp.isPresent()){
...
//        System.out.println(postalcodes);
        ...
        //employees.parallelStream().filter(x -> x.getSalary() > 117000).forEach(System.out::println);
        // accepts 1 para, returns nothing
        // stream = seq stream
        //parall stream
        // Optional -> size can be maximum of 1
